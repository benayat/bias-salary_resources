...
HOME_4GPU_CONFIG = LLMResourceConfig(
    ...
    max_num_seqs=1024,  # Higher concurrency with more GPUs
    max_num_batched_tokens=524288,  # Larger batch size
...
HPC_2H200_CONFIG = LLMResourceConfig(
    gpu_memory_utilization=0.92,        # small headroom vs 0.95
    max_model_len=512,                  # enough for your ~100-token prompts + outputs
    max_num_seqs=8,                  # high concurrency, real limiter is KV
    max_num_batched_tokens=4096,   # 2^20, fits under ~1.36M KV tokens
    block_size=16,
